...
public class GameControllerMain09 : MonoBehaviour {
	const int kUp = 0;
	const int kDown = 1;
	const int kRight = 2;
	const int kLeft = 3;
	...
	void Update (){
		...
		if (!gameClear) {
			if (isMoving) {
				...
				if (moveTime <= 0f) {
					...
					//移動回数
					Move09 m =move.GetComponent<Move09>();
					m.ClickCount++;
				}
		...
		if (Clear1 == true && Clear2 == true) {
			TimeController09 t = timer.GetComponent<TimeController09> ();
			t.time = false;
			gameClear = true;
			...
			cable2.enabled = true;
			Animator right = ClearObj.GetComponent<Animator> ();
			right.enabled = true;
			Animator fan = ClearObj2.GetComponent<Animator> ();
			fan.enabled = true;
			ClearObj3.SetActive (true);
	...
	public void DoCheck() {
		...
		while (true) {
			...
			if (panel == null) {
				float goalDist = (pos - new Vector2 (49f, 79f)).magnitude;
				if (goalDist < 5f) {
					Clear1 = true;
				} else {
					Clear1 = false;
	...
	public void DoCheck1() {
		Vector2 pos = new Vector2 (499f, 379f);
