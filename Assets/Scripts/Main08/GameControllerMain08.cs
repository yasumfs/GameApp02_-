...
public class GameControllerMain08 : MonoBehaviour {
	const int kUp = 0;
	const int kDown = 1;
	const int kRight = 2;
	const int kLeft = 3;
	...
	void Update (){
		...
		if (!gameClear) {
			if (isMoving) {
				...
				if (moveTime <= 0f) {
					...
					//移動回数
					Move08 m08 =move.GetComponent<Move08>();
					m08.ClickCount++;
				}
		...
		if (Clear1 == true && Clear2 == true) {
			...
			cable2.enabled = true;
			Animator branch = Branch.GetComponent<Animator> ();
			branch.enabled = true;
			Animator fan = Fan.GetComponent<Animator> ();
			fan.enabled = true;
			Animator fan2 = Fan2.GetComponent<Animator> ();
			fan2.enabled = true;
			Wind.SetActive (true);
	...
	public void DoCheck() {
		...
		while (true) {
			...
			if (panel == null) {
				...
				float goalDist = (pos - new Vector2 (349f, 229f)).magnitude;
	...
	public void DoCheck1() {
		Vector2 pos = new Vector2 (349f, 79f);
	...
	public void DoCheck2() {
		Vector2 pos = new Vector2 (349f, 379f);
