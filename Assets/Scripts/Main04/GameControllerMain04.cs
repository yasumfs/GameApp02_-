...
public class GameControllerMain04 : MonoBehaviour {
	const int kUp = 0;
	const int kDown = 1;
	const int kRight = 2;
	const int kLeft = 3;
		...
		if (!gameClear) {
			if (isMoving) {
				...
				if (moveTime <= 0f) {
					...
					//移動回数
					Move04 m04 =move.GetComponent<Move04>();
					m04.ClickCount++;
				}
	...
	public void DoCheck() {
		Vector2 pos = new Vector2 (500f, 72f);
		...
		while (true) {
			...
			if (panel == null) {
				...
				float goalDist = (pos - new Vector2 (60f, 402f)).magnitude;
				// 距離が一定の距離以下であればゴールにたどり着いたのでクリア
				if (goalDist < 5f) {
					TimeController04 t = timer.GetComponent<TimeController04>();
					t.time = false;
					gameClear = true;
					Animator cable = Cable.GetComponent<Animator> ();
					cable.enabled = true;
					Animator signal = ClearObj.GetComponent<Animator> ();
					signal.enabled = true;
	...
	Vector2 GetNextPosition(Vector2 currentPos, int nextDirection)
	{
		if (nextDirection == kUp) {
			return currentPos + new Vector2 (0, 110f);
		} else if (nextDirection == kDown) {
			return currentPos + new Vector2 (0, -110f);
		} else if (nextDirection == kLeft) {
			return currentPos + new Vector2 (-110f, 0f);
