...
public class GameControllerMain05 : MonoBehaviour {
	const int kUp = 0;
	const int kDown = 1;
	const int kRight = 2;
	const int kLeft = 3;
	...
	public void DoCheck() {
		Vector2 pos = new Vector2 (499f, 84f);
		...
		while (true) {
			// 現在のチェックポイントの確認
			//Debug.Log ("pos = " + pos);
			if (pos == new Vector2 (349f, 234f) && direction == kLeft){
				pos = new Vector2 (199f, 234f);
			}
			// 現在位置のパネルを取得する
			...
			if (panel == null) {
				...
				float goalDist = (pos - new Vector2 (49f, 384f)).magnitude;
				...
				// 距離が一定の距離以下であればゴールにたどり着いたのでクリア
					if (goalDist < 5f) {
						...
						fan.enabled = true;
						Animator Center = CenterPanel.GetComponent<Animator> ();
						Center.enabled = true;
						wind.SetActive (true);
