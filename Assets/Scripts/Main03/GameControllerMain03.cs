...
public class GameControllerMain03 : MonoBehaviour {
	const int kUp = 0;
	const int kDown = 1;
	const int kRight = 2;
	const int kLeft = 3;
		...
		if (!gameClear) {
			if (isMoving) {
				...
				if (moveTime <= 0f) {
					...
					//移動回数
					Move03 m03 =move.GetComponent<Move03>();
					m03.ClickCount++;
				}
	...
	public void DoCheck() {
		Vector2 pos = new Vector2 (588f, 82f);
		...
		while (true) {
			...
			if (panel == null) {
				...
				float goalDist = (pos - new Vector2 (27f, 362f)).magnitude;
				// 距離が一定の距離以下であればゴールにたどり着いたのでクリア
				if (goalDist < 5f) {
					TimeController03 t = timer.GetComponent<TimeController03>();
					t.time = false;
					gameClear = true;
	...
	Vector2 GetNextPosition(Vector2 currentPos, int nextDirection)
	{
		if (nextDirection == kUp) {
			return currentPos + new Vector2 (0, 140f);
		} else if (nextDirection == kDown) {
			return currentPos + new Vector2 (0, -140f);
		} else if (nextDirection == kLeft) {
			return currentPos + new Vector2 (-140f, 0f);
